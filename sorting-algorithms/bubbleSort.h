...
vector<Comparable> bubbleSort(vector<Comparable> vec, unsigned long &reads,
        ...
        allocations += sizeof(int) * 2;                 // numPasses, i
        ...
        while (haveSwapped) {
                ...
                for (i = 0; i + 1 < vec.size() - numPasses; ++i) {
                        // Compare items at indices i and i+1 and swap if
                        // necessary
                        reads += 2;
                        if (vec[i] > vec[i + 1]) {
                                reads += 2;
